...
**Exit gate**: on this machine, run the `.AppImage` and take a patch via the built-in updater (verifies the patch + rollback path); the same flow verified on Windows (launcher + sidecar path); a fresh clone to green CI is reproducible. **Windows sidecar MET** — measured: a 26.225 portable applied 26.226 and its dylib came out byte-identical to the published one, with the applied version recorded per-user and no version stamp written back into the bundle. Still open: the **on-device relaunch** on both platforms (each needs a desktop session; the swap itself is proven), and the **AppImage cannot stage an update at all** — Deno writes `<dylib>.update` next to the dylib, which inside an AppImage is a read-only squashfs mount (`EROFS`, os error 30, measured on a real read-only mount). The AppImage is a fresh-download artifact until the dylib can live writable.
